    ...
    if (campoBuscaAgente.value == "") {
      ...
      if (!agentes.msg) {
        ...
        for (const agente of agentes) {
          const valorBuscado = campoBuscaAgente.value.toLocaleLowerCase();
          const nomeLower = agente.nome.toLocaleLowerCase();
          const idString = String(agente.id_agente);
          
          if (nomeLower.includes(valorBuscado) || idString.includes(valorBuscado)) {
            /**
    ...
    if (campoBuscaUsuario.value == "") {
      ...
      if (!usuarios.msg) {
        for (const usuario of usuarios) {
          const valorBuscado = campoBuscaUsuario.value.toLocaleLowerCase();
          if (usuario.nome.toLocaleLowerCase().includes(valorBuscado)) {
            /**
...
if (document.querySelector(".modal-busca")) {
  document.querySelector(".modal-busca").addEventListener("click", (event) => {
    if (resultadoBusca.classList.contains("main-page-hero-resultado-visivel")) {
...
if (campoBuscaDataRelatorio) {
  ...
  campoBuscaDataRelatorio.addEventListener("input", async () => {
    window.location = "?data=" + campoBuscaDataRelatorio.value;
  });
}
...
window.addEventListener("load", async () => {
  ...
  const data = {
     ...
     * @type {string[]}
     */
    labels: ["Total Vendido", "Total Pago", "Total Extraído", "Facturações"],
    ...
    datasets: [
      {
         ...
         * @type {CanvasGradient[]}
         */
        backgroundColor: [gradVendido, gradPago, gradExtraido, gradFaturacoes],
         ...
         * @type {string[]}
         */
        hoverBackgroundColor: ["#10b981", "#3b82f6", "#ef4444", "#f59e0b"],
      },
    ],
  };
