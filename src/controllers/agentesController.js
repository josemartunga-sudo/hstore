...
class AgentesController {
  ...
  buscarFacturacoesAgentes = async (req, res) => {
    ...
    data = new Date(data);
    try {
      const response = await faturacaoService.pegarTodasFacturacoesDoAgente(
        id,
        data
      );
  ...
  buscarRelatorioDeAgente = async (req, res) => {
    ...
    data = new Date(data);
    try {
      const response = await relatorioService.pegarRelatorioDePagamentoDoAgente(
        id,
        data,
        parcela
      );
  ...
  mudarFormaPagamento = async (req, res) => {
    const { id } = req.params;
    try {
      const response = await agenteService.mudarFormaPagamento(id);
      if (! response.successo) {
        req.flash("error_msg", response.mensagem);
        return res
          .status(302)
          .redirect("/agentes/perfil/" + id);
      }
      req.flash("success_msg", response.mensagem);

      return res
        .status(response.successo ? 200 : 400)
        .redirect("/agentes/perfil/" + id);
    } catch (error) {
