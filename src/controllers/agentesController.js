...
class AgentesController {
    ...
    listarAgentesAtivos = async (req, res) => {
        try {
            ...
            if (!agentes) {
                return res.status(200).json({ msg: "Nenhum agente cadastrado!" });
            }
    ...
    buscarFacturacoesAgentes = async (req, res) => {
        ...
        data = new Date(data);
        try {
            const response = await faturacaoService.pegarTodasFacturacoesDoAgente(id, data);
            const resumoMensalDoAgente = await relatorioService.pegarResumoMensalDoAgente(id, data);
    ...
    buscarRelatorioDeAgente = async (req, res) => {
        ...
        data = new Date(data);
        try {
            const response = await relatorioService.pegarRelatorioDePagamentoDoAgente(id, data, parcela);
    ...
    criarRelatorioDoAgente = async (req, res) => {
        try {
            ...
            const dados = req.body;
            const response = await relatorioService.criarRelatorioFinalDoAgente(id_usuario, dados);

            if (response.newPage) {
                return res.status(302).json({ msg: response.mensagem, page: response.page });
            }
    ...
    /**
     * Altera o estado (ativo/inativo) de um agente.
    //PENDENTE...
    mudarEstado = async (req, res) => {
        ...
        try {
            ...
            if (!agente) {
                return res.status(404).json({ msg: "Agente não existe" });
            }

            const novoEstado = agente.estado;
            const atualizado = await agenteService.mudarEstado(id, novoEstado);
            ...
            console.error("Erro ao mudar estado do agente:", error);
            res.render("pages/error", {
                titulo: "Internal error"
            });
        }
