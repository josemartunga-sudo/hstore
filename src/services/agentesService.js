...
class AgentesService {
  ...
  async actualizarAgente(dados) {
    ...
    try {
      ...
      if (atualizado) {
        ...
        return { successo: false, mensagem: "Sub-agente não encontrado ou dados iguais." };
      }
    } catch (erro) {
   ...
   */
  async mudarFormaPagamento(idAgente) {
    try {
      ...
      if (!Facturacoes.length) {
        return { sucesso: false, mensagem: "Nenhuma facturação encontrada este mês." };
      }

      // Verifica se todas as facturas são "Mensal"
      const todasMensais = Facturacoes.every((f) => f.forma_pagamento === "Mensal");

      if (!todasMensais) {
        return { sucesso: false, mensagem: "Só é permitido alterar se todas as facturas forem Mensais." };
      }
      ...
      return { sucesso: true, mensagem: "Forma de pagamento alterada para Quinzenal com sucesso." };
    } catch (erro) {
